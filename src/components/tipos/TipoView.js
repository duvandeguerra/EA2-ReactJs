...
import { getTiposEquipos, postTiposEquipo, putTiposEquipos } from '../../services/tipoEquipoService';
...
export const TipoView = () => {
  ...
  const nuevoTipo = async (data) => {
    try {
      ...
      if (tipo) {
        const resp = await putTiposEquipos(tipo._id, data);
        console.log(resp.data);
      } else {
        const resp = await postTiposEquipo(data);
        console.log(resp.data);
      }
    ...
    <div className='container-fluid mt-3 mb-2'>
      <div className="card">
        <div className="card-header">
          <h5 className="card-title">Crear / Editar Tipo Equipo </h5>
        </div>
