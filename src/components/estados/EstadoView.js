...
import { getEstadosEquipos, postEstadosEquipos, putEstadosEquipos } from '../../services/estadoEquipoService';
...
export const EstadoView = () => {
  ...
  const nuevoEstado = async (data) => {
    try {
      ...
      if (estadoE) {
        const resp = await putEstadosEquipos(estadoE._id, data);
        console.log(resp.data);
      } else {
        const resp = await postEstadosEquipos(data);
        console.log(resp.data);
      }
    ...
    <div className='container-fluid mt-3 mb-2'>
      <div className="card">
        <div className="card-header">
          <h5 className="card-title">Crear / Editar Estado Equipo </h5>
        </div>
